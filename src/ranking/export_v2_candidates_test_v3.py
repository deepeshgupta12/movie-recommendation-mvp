...
def _load_conf_splits():
    ...
    test_p = _processed_dir() / "test_conf.parquet"

    if not train_p.exists() or not test_p.exists():
        # Keep a soft fallback for local dev parity
        # but make it explicit in logs.
        val_p = _processed_dir() / "val_conf.parquet"
        if train_p.exists() and val_p.exists():
            print(
                "[WARN] test_conf.parquet not found. "
                "Falling back to val_conf.parquet for TEST V2 export."
            )
            train_conf = pl.read_parquet(train_p)
            test_conf = pl.read_parquet(val_p)
            return train_conf, test_conf

        raise FileNotFoundError(
            "Missing confidence splits. Expected train_conf.parquet and test_conf.parquet "
            "from the confidence split pipeline."
        )

    train_conf = pl.read_parquet(train_p)
    test_conf = pl.read_parquet(test_p)
    return train_conf, test_conf


def _get_test_users(cap: int = 50000) -> List[int]:
    _, test_conf = _load_conf_splits()
    users = (
...
def main(max_k: int = 200, user_cap: int = 50000):
    print("\n[START] Exporting V2 candidates for test...")

    train_conf, _ = _load_conf_splits()
    users = _get_test_users(cap=user_cap)
    ...
    out_path = _processed_dir() / "v2_candidates_test.parquet"
